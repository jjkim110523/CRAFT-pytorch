...
def load_icdar2015_gt(dataFolder, isTraing=False):
    if isTraing:
        img_folderName = "train/images"
        gt_folderName = "train/labels"
    else:
        img_folderName = "test/images"
        gt_folderName = "test/labels"
...
def load_custom_gt(config, isTraining=True):
    """
    Custom 데이터셋을 불러오는 함수로, self.config를 활용하여 설정을 적용합니다.
    """
    ...
    if isTraining:
        ...
        img_folderName = "test/images"
        gt_folderName = "test/labels"
        sample = config.test.custom_data.test_set_size

    gt_folder_path = os.listdir(os.path.join(config.test.custom_data.test_data_dir, gt_folderName))
    
    # 샘플링 적용 (sample 값이 -1이면 전체 데이터 사용)
    if sample > 0:
        gt_folder_path = gt_folder_path[:sample]

    total_imgs_bboxes = []
    total_img_path = []
    
    for gt_path in gt_folder_path:
        gt_path = os.path.join(os.path.join(config.test.custom_data.test_data_dir, gt_folderName), gt_path)
        img_path = (
            gt_path.replace(gt_folderName, img_folderName)
            .replace(".txt", ".png")
        )
        ...
        for line in lines:
            ...
            box_info_dict["points"] = box_points
            box_info_dict["text"] = word
            box_info_dict["ignore"] = True if word == "###" else False
